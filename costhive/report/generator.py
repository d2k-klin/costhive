...
from costhive.aggregate import Report

VALID_FORMATS = {"html", "md", "json", "pdf"}

#: One accent color per savings category, used in the category breakdown chart.
_CATEGORY_COLORS = {
    "idle": "#dc2626",
    "rightsizing": "#d97706",
    "unused": "#7c3aed",
    "storage_class": "#0891b2",
    "commitment": "#2563eb",
    "off_hours": "#059669",
    "network": "#db2777",
    "untagged": "#6b7280",
...
def _env() -> Environment:
    env = Environment(
        loader=PackageLoader("costhive.report", "templates"),
        autoescape=select_autoescape(["html", "xml"]),
        trim_blocks=True,
        lstrip_blocks=True,
    )
    env.globals["category_colors"] = _CATEGORY_COLORS
...
def write_reports(
    ...
    *,
    ...
    other formats — the user still gets HTML/MD/JSON.
    """
...
def _render_weasyprint(html: str, pdf_path: str) -> None:
    try:
        ...
        raise PdfError(
            "WeasyPrint is not available. Install it with `pip install costhive[pdf]` "
