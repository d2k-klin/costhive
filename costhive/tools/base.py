...
Every FinOps tool wraps one underlying OSS binary behind the same contract:

    tool.run(ctx, workdir) -> ToolResult   # normalized savings findings + metadata

Adding a 7th tool is just a new subclass — the orchestrator, normalizer schema,
...
from costhive.models import SavingsFinding
...
def session_env(ctx: AwsContext | None) -> dict:
    ...
    environment, so a wrapped tool authenticates as exactly the identity CostHive
...
class ToolResult:
    tool: str
    status: ToolStatus
    findings: list[SavingsFinding] = field(default_factory=list)
    message: str = ""
...
class CostTool:
    """Base class. Subclasses set `name`/`binary` and implement `_run`."""

    name: str = "tool"
    binary: str = ""  # CLI executable that must be on PATH
    #: True if this tool inspects a live AWS account; False if it scans local files.
    requires_aws: bool = True

    #: Flag passed to the binary to print its version (override if non-standard).
    version_flag: str = "--version"
    #: If enabled, tool stdout/stderr lines are also printed while they run.
    show_tool_output: bool = False
    #: Elapsed-time heartbeat interval for long-running tool commands.
    ...
    def run(self, ctx: AwsContext | None, workdir: str) -> ToolResult:
        ...
        try:
            result = self._run(ctx, workdir)
        except subprocess.TimeoutExpired:
            return ToolResult(self.name, ToolStatus.ERROR, message="tool timed out", version=version)
        except Exception as exc:  # noqa: BLE001 - one tool failing must not abort the run
            return ToolResult(self.name, ToolStatus.ERROR, message=str(exc), version=version)
    ...
    def _exec(
        ...
        The return shape intentionally matches subprocess.run(). Many tool wrappers
        need stderr/stdout after completion, while users need assurance that long
        analyses are still active.
        """
        if not progress:
        ...
        return self._exec_with_progress(
            ...
            stream_output=self.show_tool_output if stream_output is None else stream_output,
    ...
    def _print_progress_heartbeat(
        ...
        activity = (
            f"last tool output {_format_elapsed(last_output_age)} ago"
            if lines_seen and last_output_age is not None
            else "waiting for tool output"
        )
        print(
