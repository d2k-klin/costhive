"""Tests for costhive.auth — credential resolution with mocked AWS (botocore Stubber).
...
class TestResolveSession:
    ...
    def test_assume_role_uses_sts(self):
        with patch("costhive.auth.boto3.Session") as mock_cls:
            ...
            assert call_kwargs["RoleSessionName"] == "costhive"
...
class TestDefaultRegions:
    ...
    def test_falls_back_to_us_east_1(self, monkeypatch):
        ...
        assert default_regions(session) == ["us-east-1"]


class TestBuildContexts:
    def test_no_role_arns_yields_single_context(self):
        with (
            patch("costhive.auth.resolve_session") as mock_resolve,
            patch("costhive.auth.verify_identity") as mock_verify,
        ):
            mock_resolve.return_value = boto3.Session(region_name="us-east-1")
            mock_verify.return_value = type(
                "Id",
                (),
                {"account_id": "123456789012", "arn": "arn:aws:iam::123456789012:user/me", "user_id": "AIDA"},
            )()
    ...
    def test_multiple_role_arns_yield_multiple_contexts(self):
        ...
        with patch("costhive.auth.build_context", side_effect=mock_build_context):
            contexts = build_contexts(
                role_arns=["arn:aws:iam::111:role/A", "arn:aws:iam::222:role/B"], external_id="shared"
            )
...
class TestDiscoverEksClusters:
    def test_returns_clusters(self):
        ...
        ctx = type("Ctx", (), {"regions": ["eu-central-1"], "client": lambda self, svc, region=None: client})()
        with stubber:
    ...
    def test_returns_empty_on_error(self):
        ...
        ctx = type("Ctx", (), {"regions": ["us-east-1"], "client": lambda self, svc, region=None: client_mock})()
        assert discover_eks_clusters(ctx) == []
