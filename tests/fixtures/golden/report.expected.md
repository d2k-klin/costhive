...
| **Prepared by** | CostHive by Mr.D · v0.0.8 |
...
> _($3,216.00/year across 9 opportunities)_
>
...
### Top 9 opportunities by dollar impact
...
| 9 | $0.00 | High | Judgment call | Rightsizing | krr | `prod-eks/payments/Deployment/payments-api/api` | Right-size Deployment payments/payments-api (api) |

## Tools

| Tool | Status | Findings | Est. savings | Version | Notes |
|------|--------|----------|--------------|---------|-------|
| steampipe | ok | 3 | $18.00 | Steampipe v2.4.4 | 5/5 checks ran; coverage: ebs unattached, eip unassociated, ebs gp2 to gp3, stopped instances with ebs, old snapshots |
| custodian | ok | 3 | $180.00 | custodian 0.9.51 | 6 policies across 2 file(s) evaluated (dry-run, no changes made); coverage: ebs unattached, eip unassociated, rds idle no connections, ec2 low utilization, ec2 untagged, gp2 volumes |
| komiser | ok | 1 | $0.00 | komiser 3.1.22 | 1 untagged cost-allocation gap(s) |
| opencost | ok | 1 | $70.00 | opencost 1.121.0 |  |
| krr | ok | 1 | $0.00 | 1.29.0 | 1 workload/container recommendation(s) imported |
...
### $0.00/mo — Right-size Deployment payments/payments-api (api)

- **Category:** Rightsizing · **Confidence:** High · **Risk:** Judgment call
- **Source tool:** krr
- **Service:** kubernetes
- **Resource:** `prod-eks/payments/Deployment/payments-api/api`
- **Why:** KRR analyzed historical Kubernetes usage and recommends request changes: cpu reduce 2 cores → 500m; memory reduce 4 GiB → 1.5 GiB.
- **Recommended action:** Apply the recommended requests in a staging environment, verify peak-load headroom, then combine with OpenCost to quantify the dollar impact.


---
_Prepared by [CostHive by Mr.D](https://github.com/d2k-klin/costhive) · CostHive v0.0.8. Savings are estimates — validate against your own usage before acting. CostHive keeps generated reports local; connected tools may call their configured services._